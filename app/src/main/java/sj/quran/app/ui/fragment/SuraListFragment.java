...
public class SuraListFragment extends Fragment {
    ...
    private QuranRow[] getSuraList() {
        ...
        boolean wantPrefix = requireContext().getResources().getBoolean(R.bool.show_surat_prefix);
        boolean wantTranslation = requireContext().getResources().getBoolean(R.bool.show_sura_names_translation);
        for (int juz = 1; juz <= JUZ2_COUNT; juz++) {
            final String headerTitle = requireContext().getString(R.string.juz2_description,
            ...
            while ((sura <= SURAS_COUNT) &&
                ...
                final QuranRow.Builder builder = new QuranRow.Builder()
                        ...
                        .withMetadata(QuranInfo.getSuraListMetaString(requireContext(), sura))
                        .withSura(sura)
